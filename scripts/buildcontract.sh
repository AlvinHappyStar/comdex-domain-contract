...
case $NETWORK in
  devnet)
    ...
    ;;
  testnet)
    NODE="https://rpc-nova.comdex.one:443"
    DENOM=ucmdx
    CHAIN_ID=comdex-novanet
    LP_TOKEN_CODE_ID=123
    WALLET="--from testnet-key"
    ADDR_ADMIN="comdex1xuu33nvxl68ts7xyy2elvp7rw54t6n6sv5v229"
    ;;
  mainnet)
    NODE="https://comdex-rpc.polkachu.com:443"
    DENOM=ucmdx
    CHAIN_ID=comdex-1
    LP_TOKEN_CODE_ID=1
    WALLET="--from testnet-key"
...
TXFLAG=" $NODECHAIN --gas-prices 0.025$DENOM  --gas 8000000 --gas-adjustment 1.3"
...
Upload() {
    ...
    UPLOADTX=$(comdex tx wasm store ./release/*.wasm $WALLET $TXFLAG --output json -y | jq -r '.txhash')
    ...
    do 
        sleep 3
        CODE_ID=$(comdex query tx $UPLOADTX $NODECHAIN --output json | jq -r '.logs[0].events[1].attributes[1].value')
    done
...
InstantiateIncentive() {
    ...
    TXHASH=$(comdex tx wasm instantiate $CODE_INCENTIVE '{}' --label "domain_$CODE_INCENTIVE" --amount 1ucmdx --admin $ADDR_ADMIN $WALLET $TXFLAG -y --output json | jq -r '.txhash')
    ...
    do
        sleep 3
        CONTRACT_ADDR=$(comdex query tx $TXHASH $NODECHAIN --output json | jq -r '.logs[0].events[2].attributes[0].value')
    done
    echo $CONTRACT_ADDR
...
ClaimRegister() {
    ...
    echo $(comdex tx wasm execute $CONTRACT_INCENTIVE '{"register": {"name": "name", "duration": 2}}' --amount 1000000ucmdx --from st $TXFLAG)
}

ClaimExtend() {
    CONTRACT_INCENTIVE=$(cat $ADDRESS_DIR"domain")
    echo $(comdex tx wasm execute $CONTRACT_INCENTIVE '{"extend": {"name": "name2", "duration": 1}}' --amount 1000000ucmdx --from st $TXFLAG)
}

WithDraw() {
    CONTRACT_INCENTIVE=$(cat $ADDRESS_DIR"domain")
    echo $(comdex tx wasm execute $CONTRACT_INCENTIVE '{"withdraw": {}}' --from testnet-key $TXFLAG -y)
}
